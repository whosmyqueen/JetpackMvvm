...
class NavigationFragment : BaseFragment<TreeViewModel, IncludeListBinding>() {
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        //初始化recyclerView
        mDatabind.includeRecyclerview.recyclerView.init(LinearLayoutManager(context), navigationAdapter).let {
    ...
    override fun createObserver() {
        ...
        appViewModel.run {
            //监听全局的主题颜色改变
            appColor.observe(this@NavigationFragment, Observer {
                setUiTheme(it, mDatabind.floatbtn, mDatabind.includeRecyclerview.swipeRefresh, loadsir)
            })
            //监听全局的列表动画改编
            appAnimation.observe(this@NavigationFragment, Observer {
